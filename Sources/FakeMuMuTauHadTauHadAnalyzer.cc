...
void FakeMuMuTauHadTauHadAnalyzer::Loop()
{
   ...
   for (Long64_t jentry=0; jentry<nentries; jentry++) {
      ...
      for (unsigned int iJet=0; iJet<recoJetPt->size(); iJet++)
      {
          ...
          if ((!massDecorrelation && recoJetDeepDiTauValuev1->at(iJet) < deepDiTauRawThreshold) || (massDecorrelation && recoJetDeepDiTauValueMDv1->at(iJet) < deepDiTauRawThreshold)) continue;
      ...
      for (unsigned int iTau=0; iTau<recoTauBoostedPt->size(); iTau++)
      {
          ...
          Tau.SetPtEtaPhiE(recoTauBoostedPt->at(iTau), recoTauBoostedEta->at(iTau), recoTauBoostedPhi->at(iTau), recoTauBoostedEnergy->at(iTau));
          ...
          for (unsigned int iTau2=iTau+1; iTau2<recoTauBoostedPt->size(); iTau2++)
          {
              ...
              {
                  Tau2.SetPtEtaPhiE(recoTauBoostedPt->at(iTau2), recoTauBoostedEta->at(iTau2), recoTauBoostedPhi->at(iTau2), recoTauBoostedEnergy->at(iTau2));
