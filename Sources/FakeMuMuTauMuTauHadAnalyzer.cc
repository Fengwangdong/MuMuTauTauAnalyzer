...
void FakeMuMuTauMuTauHadAnalyzer::Loop()
{
   ...
   for (Long64_t jentry=0; jentry<nentries; jentry++) {
      ...
      for (unsigned int iTau=0; iTau<recoTauMuonCleanedPt->size(); iTau++)
      {
          ...
          Tau.SetPtEtaPhiE(recoTauMuonCleanedPt->at(iTau), recoTauMuonCleanedEta->at(iTau), recoTauMuonCleanedPhi->at(iTau), recoTauMuonCleanedEnergy->at(iTau));
          ...
          float smallestDR = 0.8; // dR between Mu3 and tau
          bool findMu3 = false;
          ...
          for (unsigned int iMuon=0; iMuon<recoMuonPt->size(); iMuon++)
          {
              ...
              //if ((recoTauMuonCleanedPDGId->at(iTau)/fabs(recoTauMuonCleanedPDGId->at(iTau)) == (-1) * recoMuonPDGId->at(iMuon)/fabs(recoMuonPDGId->at(iMuon))) && (Tau.DeltaR(Mu3Cand) < smallestDR) && ((Tau+Mu3Cand).M() < 60.0) && (Mu3Cand.DeltaR(Mu1) > 0.4) && (Mu3Cand.DeltaR(Mu2) > 0.4) && !overlapMuTau)
              if ((recoTauMuonCleanedPDGId->at(iTau)/fabs(recoTauMuonCleanedPDGId->at(iTau)) == (-1) * recoMuonPDGId->at(iMuon)/fabs(recoMuonPDGId->at(iMuon))) && (Tau.DeltaR(Mu3Cand) < smallestDR) && (Mu3Cand.DeltaR(Mu1) > 0.4) && (Mu3Cand.DeltaR(Mu2) > 0.4) && !overlapMuTau)
              {
