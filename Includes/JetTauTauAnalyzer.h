...
// found on file: JetTauTauTreelization_100.root
//////////////////////////////////////////////////////////

#ifndef JetTauTauAnalyzer_h
#define JetTauTauAnalyzer_h
...
class JetTauTauAnalyzer : public HistoJettautau {
public :
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   JetTauTauAnalyzer(TString fileName_, TString outputDir_, float lumiScale_, float summedWeights_ = 1.0, Long_t nMaxEvents_ = 0, bool isMC_ = false, bool invertedTauIso_ = false, bool invertedTau2Iso_ = false, TString JetId_ = "TIGHT", bool massDecorrelation_ = false, double deepDiTauRawThreshold_ = 0.5, double deepDiTauRawLowerBound_ = 0.2, TString doWhatSample_ = "ZTT");
   string createOutputFileName();
   virtual ~JetTauTauAnalyzer();
...
#ifdef JetTauTauAnalyzer_cxx
JetTauTauAnalyzer::JetTauTauAnalyzer(TString fileName_, TString outputDir_, float lumiScale_, float summedWeights_, Long_t nMaxEvents_, bool isMC_, bool invertedTauIso_, bool invertedTau2Iso_, TString JetId_, bool massDecorrelation_, double deepDiTauRawThreshold_, double deepDiTauRawLowerBound_, TString doWhatSample_) : HistoJettautau() 
{
    fileName = fileName_;
    ...
    invertedTau2Iso = invertedTau2Iso_;
    JetId = JetId_;
    ...
    TString treePath = fileName + "/JetDiTauAnalyzer/objectTree";
    chain->Add(treePath);
    fChain = chain;
    Init();
}

string JetTauTauAnalyzer::createOutputFileName()
{
...
JetTauTauAnalyzer::~JetTauTauAnalyzer()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t JetTauTauAnalyzer::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t JetTauTauAnalyzer::LoadTree(Long64_t entry)
{
...
void JetTauTauAnalyzer::Init()
{
   ...
   fChain->SetBranchAddress("recoJetTriggerFlag", &recoJetTriggerFlag, &b_recoJetTriggerFlag);
...
Bool_t JetTauTauAnalyzer::Notify()
{
...
void JetTauTauAnalyzer::Show(Long64_t entry)
{
...
Int_t JetTauTauAnalyzer::Cut(Long64_t entry)
{
...
#endif // #ifdef JetTauTauAnalyzer_cxx
